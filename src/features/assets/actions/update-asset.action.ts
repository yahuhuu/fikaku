...
import { updateAsset } from "../services/update-asset.service";
...
export async function updateAssetAction(formData: FormData) {
  ...
  const result = await updateAsset(String(formData.get("id") ?? ""), inputFromFormData(formData), { userId: session.user.id, assetRepository: prismaAssetRepository, familyRepository: prismaFamilyRepository });
